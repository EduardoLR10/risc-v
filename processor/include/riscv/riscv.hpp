...
SC_MODULE(RISCV)
{
  ...
  SC_CTOR(RISCV) :
    ...
    DECODE_MUX_DBRANCH_A("DECODE_MUX_DBRANCH_A"), DECODE_MUX_DBRANCH_B("DECODE_MUX_DBRANCH_B"), DECODE_MUX_RETURN_ADDRESS("DECODE_MUX_RETURN_ADDRESS"),
    DECODE_MUX_ADDER("DECODE_MUX_ADDER"), DECODE_DFORWARD("DECODE_DFORWARD"), ID_EX("ID_EX"),
    // stage 3
    EXECUTE_ULA("EXECUTE_ULA"), EXECUTE_EFORWARD("EXECUTE_EFORWARD"), EXECUTE_ALUCONTROL("EXECUTE_ALUCONTROL"),
    ...
    // stage 4
    DATA_MEMORY("DATA_MEMORY"), MEM_WB("MEM_WB"), MUX_MEM("MUX_MEM"), MEM_MFORWARD("MEM_MFORWARD"),
    // stage 5
    WB_MUX("WB_MUX"),
    // controller
    CONTROLLER("CONTROLLER")
  {
    ...
    SC_METHOD(update);
    sensitive << is_branch << b_cond << is_jalx << is_jal << is_jalr << sel_mux_ra_dbranch << sel_mux_rb_dbranch << id_instruction << forward_a << forward_b << sel_alu_A << sel_alu_B << sel_mux_mem << wb_sel_mux;
    ...
    DECODE_MUX_DBRANCH_A.inputs.at(1)(mem_alu_out);
    ...
    DECODE_MUX_DBRANCH_B.inputs.at(1)(mem_alu_out);
    ...
    DECODE_DFORWARD.mem_wr_en(mem_wr_en);
  ...
  void update() {
    ...
    execute_mux_forwardb_sel.write((forward_b).read());
